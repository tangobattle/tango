//! Audio core, copied from `tango/src/audio/mod.rs`: a Stream trait, a
//! late-binding mux so the host output stream can outlive any one
//! session, and the MGBAStream adapter that pulls samples out of an mgba
//! thread and resamples to the host rate. The SDL backend is replaced by
//! the cpal one in `backend.rs`.
