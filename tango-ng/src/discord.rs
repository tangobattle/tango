//! Discord rich-presence client wrapper, ported from
//! `tango/src/discord.rs`. Owns a background
...
pub struct Client {
    ...
    // tango rode iced's ambient runtime; tango-ng owns one explicitly,
    // so the client keeps a Handle for its spawns.
...
impl Client {
    ...
    /// Kept for parity with tango's join-request affordance (unused
    /// until a "someone asked to join" badge lands).
    #[allow(dead_code)]
