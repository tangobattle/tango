...
//!
//! Copied from `tango/src/identity.rs`; the only edit is the config-dir
//! lookup, which goes through tango-ng's [`crate::config::config_dir`]
//! (the same ProjectDirs("net", "n1gp", "tango") directory tango uses,
//! so both frontends share one identity).
//!
//! A self-signed certificate kept in the config dir gives this install
//! a stable handle across runs. It's presented as the matchmaking
//! websocket's TLS client certificate (mTLS), so the signaling server
//! can read its SHA-256 fingerprint off the TLS handshake and recognize
...
//! lives in the app's `State` and is threaded into each connect. It's
//! best-effort: any failure degrades to "no client identity" and the
//! connection still succeeds. The server only asks for a client cert when
//! mTLS is enabled on the endpoint, so an absent identity is also the steady
//! state when mTLS is off — sending one is always safe, and not sending one
//! never breaks the dial.
