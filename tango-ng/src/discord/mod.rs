...
pub fn make_game_info(
    ...
    // tango-ng's play tab stores `&dyn tango_gamedb::Game`
    // directly (no per-app `Game` wrapper around it the way
    // legacy did), so we read `family_and_variant` straight off
    // the gamedb trait.
