...
//! [`crate::session::SinglePlayerSession`]. Owns the mgba thread
//! driven by the local ROM + save, hooks the primary traps that talk
//! to the shared `tango_pvp::battle::Match`, and spawns the background
//! match-run task that pumps remote inputs into the in-progress round.
//!
//! Copied from `tango/src/session/pvp.rs`, transformed for tango-ng:
//! the iced/tokio frame plumbing (`frame_notify` + external vbuf) is
//! replaced by the polled dirty flag the other sessions use
//! (`frame_dirty` + `read_frame` over an internal vbuf — the 16 ms
//! frame timer collects frames and polls `is_ended`, so the explicit
//! wakes are gone), games are `tango_gamesupport::GameRef`s, and the
//! in-session save-view panels (`selection::Loaded` / `save_view`)
//! are stripped — tango-ng has neither yet.
//!
...
pub struct PvpSession {
    ...
    /// line's P1/P2 tag reads it directly so it shows even between rounds, when
    ...
    /// of how often the UI polls.
    tps_counter: Arc<std::sync::Mutex<Counter>>,
    ...
    /// which is how the UI retires the latency readout (see [`Self::latency`]).
...
impl PvpSession {
    ...
    pub async fn new(
        ...
        let vbuf = Arc::new(Mutex::new(vec![
            0u8;
            crate::session::SCREEN_WIDTH as usize * crate::session::SCREEN_HEIGHT as usize * 2
        ]));
        let new_frame = Arc::new(AtomicBool::new(false));
        // The net receiver's end-of-match wake. tango pointed this at the
        // session's frame_notify (the iced subscription wake); tango-ng's
        // 16 ms frame timer polls `is_ended` every tick regardless, so the
        // ping lands nowhere — the receiver adapter just requires one.
        let end_of_match_notify = Arc::new(tokio::sync::Notify::new());
        ...
        // ends the match exactly as before.
        {
            ...
            let end_of_match_notify = end_of_match_notify.clone();
            ...
            tokio::task::spawn(async move {
                ...
                loop {
                    ...
                    *latency_counter.lock().unwrap() = None;
                    ...
                    // Rebuild. (tango ticked a ~30 fps redraw wake here so the
                    // give-up bar drained while the paused emulator produced no
                    // frames; tango-ng's frame timer polls `reconnect_progress`
                    // on its own, so the coordinator just awaits the rebuild.)
                    let rebuilt = rebuild_connection(&recipe, deadline, &cancel).await;
                ...
                // telemetry line retires, mark the frame dirty so the UI tick
                // redraws + re-checks `is_ended` promptly, finalize the replay
                // iff the match reached its end screen, and release the match
                // handle.
                *latency_counter.lock().unwrap() = None;
                new_frame.store(true, Ordering::Release);
        ...
        let tps_counter = Arc::new(std::sync::Mutex::new(Counter::new(60)));
        ...
        // Completion / EndOfMatch handling. Runs on the live core's
        // frame_callback: returns whether the match has completed (caller
        // pauses the live emulator if so) and fires the EndOfMatch packet
        // exactly once on the edge. (tango also armed a grace-window wake
        // here so `is_ended` was re-checked with the emulator paused;
        // tango-ng's frame timer polls it every tick, so no wake is needed.)
        let handle_completion = {
            ...
            move || -> bool {
                ...
                // grace deadline and fire our EndOfMatch exactly once. The
                // `None → Some` transition under the lock is the guard
    ...
    pub fn game(&self) -> tango_gamesupport::GameRef {
        self.local_game
    }

    /// This side's player index (P1 = 0, P2 = 1) for the match. Stable across
    /// rounds, so the status line's P1/P2 tag reads it directly rather than
    ...
    /// — see [`crate::session::SinglePlayerSession::set_joyflags`]).
    ...
    /// clears the counter). The UI keys the latency readout off this: `None`
...
/// Both sides' resolved (patched) ROM images for a PvP match. Built by
/// [`resolve_pvp_roms`] on the caller's thread — tango-ng's scanned rom
/// map is a plain `HashMap` on the UI state (not tango's shared
/// `Scanners`), so resolution happens synchronously before the async
/// build instead of inside it — and consumed by [`spawn_pvp`].
pub struct PvpRoms {
    pub local_game: crate::rom::GameRef,
    pub local_rom: Vec<u8>,
    pub remote_game: crate::rom::GameRef,
    pub remote_rom: Vec<u8>,
}

/// Resolve the two ROMs the match runs: the local side from the user's
/// selection, the remote side from the peer's Settings `game_info` —
/// each with its BPS patch reapplied from disk. The resolution half of
...
pub fn resolve_pvp_roms(
...
) -> anyhow::Result<PvpRoms> {
    let local_rom_raw = roms
        .get(&local_game)
        .ok_or_else(|| anyhow::anyhow!("local rom not scanned"))?;
    let local_rom = if let Some((name, version)) = local_patch.as_ref() {
        crate::patch::apply_patch_from_disk(local_rom_raw, local_game, patches_path, name, version)?
    } else {
        local_rom_raw.clone()
    };
    ...
    let remote_rom_raw = roms
        .get(&remote_game)
        .ok_or_else(|| anyhow::anyhow!("remote rom not scanned"))?;
    let remote_rom = if let Some(p) = remote_gi.patch.as_ref() {
        crate::patch::apply_patch_from_disk(remote_rom_raw, remote_game, patches_path, &p.name, &p.version)?
    } else {
        remote_rom_raw.clone()
    };
...
/// Build the live PvP session from the netplay handoff data plus the
/// pre-resolved ROMs. Async because `PvpSession::new` awaits the lobby
/// loop's receiver handoff — spawn it on the tokio runtime and collect
/// the result over a channel (never `block_on` from a thread that also
/// drives mgba). Port of tango's `session::spawn_pvp` (mod.rs:853-988)
/// minus the `selection::Loaded` construction (tango-ng has no in-match
/// save-view panels), with ROM resolution split into
/// [`resolve_pvp_roms`].
pub async fn spawn_pvp(
    ...
    audio_binder: crate::audio::LateBinder,
) -> anyhow::Result<PvpSession> {
    PvpSession::new(
        roms.local_game,
        Arc::new(roms.local_rom),
        roms.remote_game,
        Arc::new(roms.remote_rom),
        pre_match,
...
///
/// [`Channels::from_signaling`]: crate::net::channel::Channels::from_signaling
...
/// Small rolling-window timing counter for the TPS readout, marked once
/// per frame callback (emulator-rate) so the reading is independent of
/// UI refresh rate. Port of `tango/src/stats.rs::Counter` — tango-ng
/// has no stats module; this session is its only consumer.
struct Counter {
...
impl Counter {
    ...
    fn mean_duration(&self) -> std::time::Duration {
        if self.marks.len() < 2 {
