...
impl OpenSave {
    ...
    pub fn from_patched_rom(
        ...
        Self::from_model(
            tango_savemodel::SaveModel::from_patched_rom(game, rom, save_path, save, applied_patch),
            save_ui,
        )
    }
    ...
    pub fn from_model(model: tango_savemodel::SaveModel, save_ui: &'static dyn crate::save_ui::SaveUi) -> Self {
        ...
        for g in tango_library::game::games_in_family(family) {
        ...
        let logos: Vec<(u32, u32, iced_image::Handle)> = logo_order
            .into_iter()
            .filter_map(tango_library::game::from_gamedb_entry)
            .map(|gi| {
...
fn build_navicust_render(
    ...
    let lang = tango_library::game::region_to_language(game.region());
