...
/// A game's ROM-derived tables, as the shared views consume them.
/// Every method defaults to "this game has none of these", so an
...
/// only, no `load_rom_assets_fn`): every default answer, i.e. no
/// tables at all. Loading such a save bakes from these, so the editor
/// shell renders its empty state instead of the game needing a
/// no-assets path.
