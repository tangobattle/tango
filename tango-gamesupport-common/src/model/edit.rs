...
pub fn apply_chip_edit(save: &mut SaveModel, edit: ChipEdit) {
    ...
    match edit {
        ...
        ChipEdit::MoveChip { from, to } => {
            ...
            let remap = |i: usize| crate::model::rules::reorder_index(i, from, to);
            write_folder(
                &mut *chips,
                folder_idx,
                &new_chips,
        ...
        ChipEdit::SetChip { slot, chip } => {
            // Direct slot write — no compaction. Slot validation is the
            // view's own: it keeps any cross-referenced state (BCC's
            // folder-entry back-references) consistent inside
            // set_chip/clear_chip, and may address slots past
            // `folder_size` (BCC's navi socket).
            match chip {
                Some(chip) => {
    ...
    match edit {
        NavicustEdit::AddPart(part) => {
            ...
            let copies = (0..nc.count())
                ...
                .count();
            if copies >= crate::model::rules::MAX_COPIES_PER_PART {
                return;
            }
...
pub fn apply_navi_edit(save: &mut SaveModel, edit: NaviEdit) -> Invalidation {
    ...
    crate::model::refresh_editability(save);
