...
pub fn render_navicust_edit<'a>(
    ...
    let materialized = crate::dataview::navicust::materialize(v.as_ref(), [rows, cols], assets);
...
/// Localized label for a [`NavicustSort`] picker entry (see
/// `folder::library_sort_label` for why it's a free function).
pub fn navicust_sort_label(sort: NavicustSort, lang: &LanguageIdentifier) -> String {
    match sort {
...
pub fn as_image(loaded: &OpenSave) -> Option<image::RgbaImage> {
    ...
    let lang = crate::editor::loaded::region_language(loaded.game.region());
