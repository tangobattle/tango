...
///
/// [`GroupedAutoBattleData`]: crate::dataview::auto_battle_data::GroupedAutoBattleData
fn abd_grouped_sections(
    lang: &LanguageIdentifier,
    grouped: &crate::dataview::auto_battle_data::GroupedAutoBattleData,
...
pub fn render_auto_battle_data_edit<'a>(
            ...
            .enumerate()
    {
        ...
        let is_standard = assets
            .chip(id)
            .map(|i| matches!(i.class(), crate::dataview::rom::ChipClass::Standard))
            .unwrap_or(false);
    ...
    let lib_header = library_header(
        ...
        auto_battle_data_sort_label,
        Action::AutoBattleDataSortChanged,
    );
    editor_panes(deck_pane, editor_pane(lib_header, lib))
}

/// Localized label for an [`AutoBattleDataSort`] picker entry (see
/// `folder::library_sort_label` for why it's a free function).
pub fn auto_battle_data_sort_label(sort: AutoBattleDataSort, lang: &LanguageIdentifier) -> String {
    match sort {
