//! Headless probe: boot two exe45 games as a lockstep pair, prime both
...
//!
//! Exits 0 once both games report a live battle with telemetry present
//! for 120 consecutive pair ticks; exits 1 on timeout, dumping the last
//! menu state bytes from both cores.
...
fn main() {
    ...
    for t in 0..max_ticks {
        // PVP_PROBE_WIGGLE: after both games report primed, mash a
        // deterministic key pattern (distinct per core) to exercise the
        // battle like real play would.
        let mut keys = [0u32; 2];
        if std::env::var("PVP_PROBE_WIGGLE").is_ok() && primed_at.is_some() {
            for (i, k) in keys.iter_mut().enumerate() {
                *k = ((t / 5).wrapping_mul(2654435761) >> i) & 0x3f3;
            }
        }
        let slices = pair.tick(&keys);
        ...
        if live_streak == 120 && std::env::var("PVP_PROBE_WIGGLE").is_err() {
    ...
    println!(
        "TIMEOUT after {max_ticks} ticks: menu0={:02x?} menu1={:02x?}",
        pvp0.debug_menu_state(pair.core_mut(0)),
        pvp1.debug_menu_state(pair.core_mut(1)),
    );
    std::process::exit(1);
