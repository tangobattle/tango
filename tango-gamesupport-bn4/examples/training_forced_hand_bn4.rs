...
fn pvp_for(rom: &[u8]) -> &'static pvp::Pvp {
    match &rom[0xac..0xb0] {
        ...
        code => panic!("not a bn4 rom (code {:02x?})", code),
    }
}
...
fn main() {
    ...
    for _ in 0..30_000u32 {
        ...
        for p in 0..2 {
            ...
            keys[p] = match phase[p] {
                PlayerPhase::Custom { since } if since >= DEBOUNCE => {
                    ...
                    if organic && script < 200 {
                        // TFH_PICKSEQ=up|down|left|right|none: nudge the
                        // cursor off wherever it starts, then take chips.
                        // Each 40-tick cycle is: direction x3, gap, A x3.
                        let dir = match std::env::var("TFH_PICKSEQ").as_deref().unwrap_or("none") {
                            "up" => tango_match::keys::UP,
                            "down" => tango_match::keys::DOWN,
                            "left" => tango_match::keys::LEFT,
                            "right" => tango_match::keys::RIGHT,
                            _ => 0,
                        };
                        match script % 40 {
                            0..=2 => dir,
                            20..=22 => tango_match::keys::A,
                            _ => 0,
                        }
                    } else {
        ...
        if turns > 0 && turns == asserted_turns {
            for p in 0..2usize {
                let base = 0x02035cb0 + p as u32 * 0x50;
            ...
            if tick >= a.parse().unwrap() && tick <= b.parse().unwrap() {
                ...
                for i in 0..2 {
                    ...
                    line.push_str(&format!(
                        ...
                        core.raw_read_8(0x02036440, -1),
                        core.raw_read_8(0x02036441, -1),
                        core.raw_read_16(0x02035cb0, -1),
                    ));
                }
                println!("{line}");
            }
        ...
        if let Ok(list) = std::env::var("TFH_SNAP") {
            if list.split(',').any(|t| t.trim() == tick.to_string()) {
                for i in 0..2 {
                    if let Some(buf) = pair.video_buffer(i) {
                        let img = image::RgbImage::from_fn(240, 160, |x, y| {
                            ...
                            image::Rgb([((v & 31) as u8) << 3, (((v >> 5) & 31) as u8) << 3, (((v >> 10) & 31) as u8) << 3])
                        });
                        img.save(format!("snap_t{tick}_c{i}.png")).unwrap();
                        println!("[{tick:5}] snapped core {i}");
                    }
                }
            }
        }

        // TFH_TILES: where the two units stand — a row-firing chip
        // that misses deals 0 for reasons that have nothing to do with
        // the block.
        if std::env::var("TFH_TILES").is_ok() && tick % 60 == 0 {
            if let Some(o) = frontier[0].poll(pair.core_mut(0), &tango_match::telemetry::EventSink::new(), 0) {
                println!("[{tick:5}] tiles p0={:?} p1={:?}", o.units[0].tile, o.units[1].tile);
            }
        }
