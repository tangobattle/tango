//! Headless probe: boot two BN5 games as a lockstep pair, prime both
...
fn main() {
    ...
    for t in 0..max_ticks {
        ...
        if primed_at.is_none() && pvp0.debug_battle_state(pair.core_mut(0)) != [0; 8] {
            primed_at = Some(t);
        }
        ...
        if t % 60 == 0 {
            ...
            let boot0 = pvp0.debug_boot_state(pair.core_mut(0));
            println!(
                "[{t:5}] boot0 ss={:02x?} title={:02x?} menu={:02x?}",
                boot0.0, boot0.1, boot0.2
            );
