...
/// The GBA engine, seen through the engine-neutral surface.
///
/// This is what lets a host drive a cable match and a DS wireless match
/// with the same code: everything below differs, everything here does
/// not.
impl crate::RunningMatch for Match {
    fn advance(&mut self, local_keys: u32) -> Result<(u32, u32, i16), crate::Error> {
        let (outgoing, _report) = Match::advance(self, local_keys)?;
    ...
    fn frame(&mut self) -> Option<Vec<u8>> {
        // Cores render BGR555; hosts are promised RGBA8, so the
        // expansion happens here rather than leaking the console's
        // native format outward.
