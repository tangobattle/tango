//! Time-sync throttler. Converts an engine's raw per-frame skew
//! (`local_advantage - remote_advantage`, what a match reports as
//! `skew`) into a slowdown in fps below the base rate; the host turns that into an
