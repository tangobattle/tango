...
//! [`crate::session::training::TrainingSession`]. Pure setup UI; the
//! launch itself rides [`Effect::StartTraining`] up to the App.
...
impl State {
    ...
    pub(super) fn training_setup_modal<'a>(
        ...
        let mt_options: Vec<widgets::Choice<(u8, u8)>> = mt_table
            .iter()
            .enumerate()
            .flat_map(|(mode, &subtypes)| {
                (0..subtypes).map(move |sub| ((mode as u8), (sub as u8)))
            })
            .map(|(mode, sub)| {
