...
pub enum Message {
    ...
    HpStatsPartial(std::path::PathBuf, tango_match_mgba::analysis::MatchStats),
    ...
    /// installs the ROM).
    HpStatsLoaded(std::path::PathBuf, Option<tango_match_mgba::analysis::MatchStats>),
...
/// [`tango_match_mgba::analysis::MatchStats`] arrive.
pub struct HpChart {
                ...
                if !self.hp_charts.contains_key(&p) && !self.hp_pending.contains(&p) {
                        ...
                        crate::library::replays::load_match_stats(&config.cache_path(), &config.replays_path(), &p)
                    {
                        ...
                        self.hp_charts.insert(
                            p.clone(),
                            HpChart::new(
                                &tango_match_mgba::analysis::MatchStats { rounds: vec![] },
                                self.loaded.as_ref(),
