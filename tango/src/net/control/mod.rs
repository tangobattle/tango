//! Control plane: the reliable lobby/handshake protocol.
//!
//! The [`protocol`] `Packet` types, the [`negotiate`] version handshake, and
//! the typed send helpers that frame `Packet`s onto the reliable transport.
...
//!
//! The `Packet` helpers are inherent methods on the transport [`Sender`] /
//! [`Receiver`], defined here rather than in [`super::transport`] so the
//! transport layer stays oblivious to packet contents.
...
/// Reliable-channel `Packet` send helpers, layered on the raw transport
/// ([`Sender::send_raw`]).
impl Sender {
    ...
    // `super::data::InMatchTx`), so their old send helpers are gone.
}

/// Reliable-channel `Packet` receive: decode one framed message.
impl Receiver {
