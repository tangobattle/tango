...
pub struct State {
    ...
    /// Training-only: the dummy's screen while the user possesses it,
    /// drawn as a picture-in-picture inset by the session view. `None`
    /// whenever the dummy view isn't live. Rebuilt alongside
...
pub enum Message {
    ...
    /// transport bar's PiP button). Training's PiP is possession-driven
    /// instead, so this is inert there.
    TogglePip,
    ...
    fn update_inner(&mut self, msg: Message, mapping: &crate::input::Mapping) -> iced::Task<Message> {
        match msg {
            ...
            Message::Input(ev) => {
                ...
                // Training hotkeys (possess/record/playback, restart,
                // checkpoint save/load, pause, frame advance): rebindable
                // via the Mapping, consulted before the held set records
                // this press so OS key-repeat is filtered. Non-mgba keys,
                // so whatever falls through to the joyflag pipeline below
                // is inert.
            ...
            Message::UpdateFramebuffer => {
                ...
                if let Some(session) = self.active.as_ref() {
                    ...
                    if session.is_ended() {
                        ...
                        // Picture-in-picture: training shows the dummy's
                        // screen while possessed; replays show the opponent's
                        // screen while the bar toggle is on.
