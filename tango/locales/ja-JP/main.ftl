...
training-pip = 相手の画面
training-opponent-view = 相手画面の表示
opponent-view-off = オフ
opponent-view-picture-in-picture = ピクチャーインピクチャー
opponent-view-stack-horizontally = 横に並べる
opponent-view-stack-vertically = 縦に並べる
...
lobby-gamemode-unavailable = ゲームモードを利用できません
lobby-gamemode-error = ゲームモードを読み込めませんでした: { $error }
...
session-build-warning-show-violations = 違反内容を表示
session-build-warning-hide-violations = 違反内容を隠す
...
playback-speed = 速度
playback-input-display = 入力表示 (I)
playback-pip = 相手の画面
playback-opponent-view = 相手画面の表示
playback-swap-perspective = 相手の視点 (Tab)
...
package-editor-title = パッケージエディター
package-editor-undo = 元に戻す
package-editor-redo = やり直す
package-editor-save = 保存
package-editor-save-as = 名前を付けて保存…
package-editor-reload = パッケージを再読み込み
package-editor-reload-dirty = パッケージを再読み込みする前に変更を保存してください。
package-editor-read-only = このドキュメントは読み取り専用です。
package-editor-saved = 保存しました。
package-editor-close-prompt = 閉じる前に変更を保存しますか？
package-editor-save-close = 保存して閉じる
package-editor-discard = 変更を破棄
package-editor-keep-editing = 編集を続ける
package-editor-error = 操作を完了できませんでした：{ $error }
package-editor-unknown-action = 不明なホストアクション：{ $id }
package-editor-not-configured = エディターが設定されていません。
package-editor-no-save-path = このセーブデータには保存先のファイルがありません。

## Packages
tab-packages = パッケージ
packages-search = パッケージを検索…
packages-install = パッケージをインストール…
packages-refresh = パッケージを再スキャン
packages-open-folder = パッケージフォルダーを開く
packages-legacy-patches = 従来のパッチ
packages-back = パッケージに戻る
packages-working = パッケージを読み込んで確認しています…
packages-operation-failed = 操作を完了できませんでした：{ $error }
packages-empty = 検索に一致するパッケージがありません。
packages-unavailable = 利用不可
packages-select-prompt = パッケージを選択すると、提供する機能と依存関係を表示します。
packages-issues = パッケージの問題
packages-reveal = パッケージのファイルを表示
packages-remove = インストール済みのバージョンを削除
packages-required-by = 次のパッケージが必要としています：{ $packages }
packages-exports = 提供する機能
packages-editor = エディター
packages-gamemode = ゲームモード
packages-telemetry = テレメトリー
packages-default-export = { $name }（デフォルト）
packages-library = ライブラリ
packages-dependencies = 依存関係
packages-no-dependencies = 依存関係なし
packages-missing-dependency = { $package }（未インストール）
packages-source-bundled = Tango に同梱
packages-source-development = 開発用フォルダー
packages-source-installed = インストール済みアーカイブ
