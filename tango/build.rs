...
fn generate_rc(icon_path: &str) -> Result<String, Box<dyn std::error::Error>> {
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if target_os == "windows" {
        // `icon.ico` is produced by `win/build.sh` at release time and
        // isn't checked in, so we only embed the resource when the icon is
        // present — local source builds skip it silently.
        let icon_file = std::path::Path::new(&std::env::var("CARGO_MANIFEST_DIR")?).join("icon.ico");
        if icon_file.exists() {
            // Render `resource.rc` into OUT_DIR to keep the source tree
            // clean. Since the .rc no longer sits next to the icon, it
            // references it by absolute path with forward slashes (RC
            // string literals treat `\` as an escape; both rc.exe and
            // windres accept `/`).
            let icon_path = icon_file.to_string_lossy().replace('\\', "/");
            let rc_path = std::path::Path::new(&std::env::var("OUT_DIR")?).join("resource.rc");
            std::fs::write(&rc_path, generate_rc(&icon_path)?)?;
