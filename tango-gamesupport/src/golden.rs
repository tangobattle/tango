//! Shared determinism harness for the per-game golden-replay suites.
//!
//! Each `tango-gamesupport-<game>` crate keeps its own golden replays
//! (`*.tangoreplay` + sidecar `*.tangoreplay.expected`) under
//! `tests/golden/` and drives them through [`run`], passing its own
//! registry slice. For each replay we decode it, look up its local +
//! remote ROMs by family/variant against a directory of `*.gba` files,
//! and drive the replay end-to-end through a stepper + shadow pair,
//! computing a determinism fingerprint to compare against the sidecar.
//!
//! The fingerprint captures:
//!   - shadow_packets_sha256 — SHA256 of every shadow-generated remote
//!     packet across all rounds.
//!   - final_ram_sha256 — SHA256 of the playback core's WRAM + IWRAM at
//!     the deterministic moment all replay inputs are consumed. The RTC
//!     is pinned via `Core::set_rtc_fixed` to the replay's timestamp so
//!     RTC-reading games (e.g. exe45) hash stably.
...
use crate::{Family, GameRef};

fn workspace_root() -> PathBuf {
    // Evaluated against this crate (tango-gamesupport); its parent is the
    // workspace root regardless of which game crate drives the harness.
...
/// (family, variant) -> verified ROM bytes. [`crate::detect`] validates
/// the CRC32, so a hit guarantees the bytes match a known game.
fn discover_roms(games: &[GameRef]) -> HashMap<(&'static str, u8), Vec<u8>> {
    ...
    for entry in entries.flatten() {
        ...
        if let Some(g) = crate::detect(games, &bytes) {
...
fn resolve_side(
    ...
    label: &str,
) -> anyhow::Result<GameRef> {
    let info = side
        .and_then(|s| s.game_info.as_ref())
        .ok_or_else(|| anyhow::anyhow!("missing {} game info", label))?;
    crate::find_by_family_and_variant(games, &info.rom_family, info.rom_variant as u8)
...
fn run_one(games: &[GameRef], path: &Path, roms: &HashMap<(&'static str, u8), Vec<u8>>, bless: bool) -> CaseResult {
    ...
    let local_game = match resolve_side(games, replay.metadata.local_side.as_ref(), "local") {
        Ok(v) => v,
        Err(e) => return CaseResult::Fail(e),
    };
    let remote_game = match resolve_side(games, replay.metadata.remote_side.as_ref(), "remote") {
        Ok(v) => v,
        Err(e) => return CaseResult::Fail(e),
    };
    ...
    let computed = std::panic::catch_unwind(std::panic::AssertUnwindSafe(|| {
        compute_fingerprint(local_rom, remote_rom, &replay, local_game.hooks, remote_game.hooks)
    }));
...
/// Run the golden suite for `families` against the `*.tangoreplay`
/// fixtures in `golden_dir`. Panics on any failure; skips replays whose
/// ROMs (or the whole ROM dir) aren't present. Call from a `#[test]` in
/// each `tango-gamesupport-<game>` crate, passing its `FAMILIES` and its
/// own `tests/golden` directory.
pub fn run(families: &[&'static Family], golden_dir: &Path) {
    let games = crate::games_of(families);
    let games = games.as_slice();
    let roms = discover_roms(games);

    let mut replays: Vec<PathBuf> = std::fs::read_dir(golden_dir)
        .unwrap_or_else(|e| panic!("failed to read {}: {}", golden_dir.display(), e))
        .flatten()
        .map(|e| e.path())
    ...
    for replay_path in &replays {
        ...
        match run_one(games, replay_path, &roms, bless) {
            CaseResult::Ok => {
