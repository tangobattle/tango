//! The match-analysis widgets: the continuous-timeline HP graph (with
//! its per-side chip-event lanes), round outcome marks, and the
//! stats-to-chart cooking that resolves chip names/icons through a
//! loaded save. Gamesupport-flavored — they read match stats and
//! [`crate::loaded::OpenSave`] — so they live here rather than in the
//! game-agnostic `tango-ui` toolkit; tango re-exports them alongside
//! `tango_ui::widgets` so call sites read the same.
