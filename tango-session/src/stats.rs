...
//! [`tango_match_mgba::analysis::MatchStats`] live on disk. Written at match
...
pub fn load_match_stats(
    ...
    replays_path: &std::path::Path,
    replay_path: &std::path::Path,
) -> Option<tango_match_mgba::analysis::MatchStats> {
    let f = std::fs::File::open(stats_path(cache_path, replays_path, replay_path)).ok()?;
    tango_match_mgba::analysis::MatchStats::read(std::io::BufReader::new(f)).ok()
}
