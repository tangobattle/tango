...
pub struct Request<'a> {
    ...
    pub config: &'a tango_match::playback::BootConfig,
...
impl<W: Writer> Export<W> {
    ...
    pub fn new(
        ...
        let mut playback = tango_match::playback::Playback::new(config, Arc::new(inputs.to_vec()), &lifecycle)?;
        ...
        for (slot, &core_idx) in order.iter().enumerate() {
            ...
            if should_write {
                ...
                if let Some(fb) = pair.video_buffer(core_idx) {
                    tango_dataview::rom::bgr555_to_rgba8(fb, &mut self.vbuf);
                    if self.twosided {
