...
pub struct Offsets {
    ...
    emblem_icons_palette_pointers: u32,
    /// Flat `[u16; NUM_NAVIS]` of each navi's intrinsic base max HP (0 for the
    /// unused roster slots).
    navi_hp_table: u32,
}
...
pub static BR4J_00: Offsets = Offsets {
    ...
    emblem_icons_palette_pointers:  0x080219f4,
    navi_hp_table:                  0x08000f94,
};
...
impl Navi<'_> {
    ...
    fn try_emblem(&self) -> Option<image::RgbaImage> {
        let tiles = self.assets.mapper.get(bytemuck::pod_read_unaligned::<u32>(
            self.assets
                .mapper
                .get(self.assets.offsets.emblem_icons_pointers)
                .get(self.id * 4..)?
        ...
        let palettes = self.assets.mapper.get(bytemuck::pod_read_unaligned::<u32>(
            self.assets
                .mapper
                .get(self.assets.offsets.emblem_icons_palette_pointers)
                .get(self.id * 4..)?
                .get(..std::mem::size_of::<u32>())?,
        ));
        let palette = bytemuck::pod_read_unaligned::<tango_gamesupport_common_dataview::rom::Palette>(
            palettes.get(..std::mem::size_of::<tango_gamesupport_common_dataview::rom::Palette>())?,
        );
        Some(tango_gamesupport_common_dataview::rom::apply_palette(
            paletted, &palette,
        ))
...
impl<'a> tango_gamesupport_common_dataview::rom::Navi for Navi<'a> {
    fn base_max_hp(&self) -> Option<u16> {
        let hp = bytemuck::pod_read_unaligned::<u16>(
            &self.assets.mapper.get(self.assets.offsets.navi_hp_table)[self.id * std::mem::size_of::<u16>()..]
                [..std::mem::size_of::<u16>()],
        );
        // The unused roster slots (ids 13/14) store 0; treat that as "no entry"
        // so callers fall back to the save.
        (hp != 0).then_some(hp)
    }
...
impl tango_gamesupport_common_dataview::rom::Assets for Assets {
    ...
    fn navi(&self, id: usize) -> Option<Box<dyn tango_gamesupport_common_dataview::rom::Navi + '_>> {
        if id >= self.num_navis() || id == 13 || id == 14 {
            return None;
        }
    ...
    fn navi_order(&self) -> &[&[usize]] {
        // Every navi except the two unused slots (13, 14), in the order the
        // navi-edit grid lays them out.
        const NAVI_ORDER: &[&[usize]] = &[
            &[0, 1, 2, 8, 5, 12],
            &[3, 4, 11, 6, 9, 10, 7],
            &[15, 18, 19, 20, 21, 17, 16, 22],
        ];
        NAVI_ORDER
    }
}
