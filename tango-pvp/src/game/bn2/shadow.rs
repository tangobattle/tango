...
pub(super) fn traps(hooks: &super::Hooks, shadow_state: crate::shadow::State) -> Vec<Trap> {
    ...
    vec![
        ...
        (hooks.offsets.rom.main_read_joyflags, {
            ...
            // (non-Clone) timer. Enabled only when the match set a limit (replay
            // reconstruction = None, so the golden suite stays byte-identical).
            ...
            Box::new(move |mut core: mgba::core::CoreMutRef| {
                ...
                // Chip-select cap: pin the remote peer's screen onto the close
                // path so the shadow predicts the remote's forced close instead
                // of mispredicting it (rollback churn).
