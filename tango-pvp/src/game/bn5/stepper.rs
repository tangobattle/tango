...
pub(super) fn traps(hooks: &super::Hooks, stepper_state: crate::stepper::State) -> Vec<Trap> {
    vec![
        ...
        (hooks.offsets.rom.main_read_joyflags, {
            ...
            // chip-select timer lives in this closure (which persists for the
            // match), behind a RefCell for &mut access. PROTOTYPE: hardcoded
            // limit + enabled only on the live fastforward path (replay leaves it
            // disabled, so the golden suite stays byte-identical). The confirm (A)
            // rides the recorded local input.
