...
//! `Match`/netcode logic.
//!
//! No comm-menu / RNG / round-driving traps: the core may sit at the (hidden)
//! comm screen until the first frame is published, at which point the callback
//! load drops it straight into the battle loop.
    ...
    vec![
        (hooks.offsets.rom.in_battle_call_handle_link_cable_input, {
            ...
            Box::new(move |mut core: mgba::core::CoreMutRef| {
                ...
                // 2 = "input already copied" — the loaded state carries the rx
                // data, so the game consumes it without a real SIO exchange.
