...
//! whose first-commit / comm-menu-settings paths seed rng1 (local) and rng2
//! (shared) identically. The earlier games (BN1–BN3) keep their own per-game
//! `rng.rs` with different generators and signatures.
...
pub(crate) fn pick_rng_states(rng: &mut impl rand::Rng, is_offerer: bool) -> (u32, u32) {
