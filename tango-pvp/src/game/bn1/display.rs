...
//!
//! bn1's in-battle comm goes through the `send_and_receive` calls rather than a
//! single link-cable handler, so the neutering mirrors the primary path's SIO
//! bypass (skip the call, report linking success, stage the canned init rx) but
//! drops all the `Match`/round/netcode logic — the loaded state already carries
//! the per-tick state we want to render.
