...
folder-illegal-copies = 同名晶片數量（{ $used }）超過了上限 { $limit }。
folder-illegal-navi = 領航員晶片數量（{ $used }）超過了上限 { $limit }。
folder-illegal-mega = MEGA晶片數量（{ $used }）超過了上限 { $limit }。
folder-illegal-giga = GIGA晶片數量（{ $used }）超過了上限 { $limit }。
folder-illegal-dark = 黑暗晶片數量（{ $used }）超過了上限 { $limit }。
folder-illegal-for-game = 此晶片在該遊戲中不可用。
folder-illegal-regular-memory = 常規晶片占用 { $used }MB；上限為 { $limit }MB。
folder-illegal-tag-memory = 搭檔晶片共占用 { $used }MB；上限為 { $limit }MB。
folder-cannot-add-full = 無法新增：卡組已滿。
folder-add-exceeds-copies = 新增此晶片將超過同名晶片數量上限（{ $limit }）。
folder-add-exceeds-navi = 新增此晶片將超過領航員晶片數量上限（{ $limit }）。
folder-add-exceeds-mega = 新增此晶片將超過MEGA晶片數量上限（{ $limit }）。
folder-add-exceeds-giga = 新增此晶片將超過GIGA晶片數量上限（{ $limit }）。
folder-add-exceeds-dark = 新增此晶片將超過黑暗晶片數量上限（{ $limit }）。
save-edit-sort = 排序
