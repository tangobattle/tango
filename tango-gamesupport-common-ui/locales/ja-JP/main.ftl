...
folder-illegal-copies = コピー数（{ $used }）が上限{ $limit }枚を超えています。
folder-illegal-navi = ナビチップ数（{ $used }）が上限{ $limit }枚を超えています。
folder-illegal-mega = メガチップ数（{ $used }）が上限{ $limit }枚を超えています。
folder-illegal-giga = ギガチップ数（{ $used }）が上限{ $limit }枚を超えています。
folder-illegal-dark = ダークチップ数（{ $used }）が上限{ $limit }枚を超えています。
folder-illegal-for-game = このチップはこのゲームでは使用できません。
folder-illegal-regular-memory = レギュラーチップは{ $used }MB使用しています。上限は{ $limit }MBです。
folder-illegal-tag-memory = タッグチップは{ $used }MB使用しています。上限は{ $limit }MBです。
folder-cannot-add-full = 追加できません：フォルダがいっぱいです。
folder-add-exceeds-copies = このチップを追加すると、コピー数の上限{ $limit }枚を超えます。
folder-add-exceeds-navi = このチップを追加すると、ナビチップの上限{ $limit }枚を超えます。
folder-add-exceeds-mega = このチップを追加すると、メガチップの上限{ $limit }枚を超えます。
folder-add-exceeds-giga = このチップを追加すると、ギガチップの上限{ $limit }枚を超えます。
folder-add-exceeds-dark = このチップを追加すると、ダークチップの上限{ $limit }枚を超えます。
save-edit-sort = 並べ替え
