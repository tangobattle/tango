...
pub fn render_folder<M: 'static>(lang: &LanguageIdentifier, loaded: &OpenSave, grouped: bool) -> Element<'static, M> {
    ...
    let chips: Vec<Option<crate::dataview::save::Chip>> =
    ...
    type Item = (Option<crate::dataview::save::Chip>, GroupedChip);
...
/// (not a method) because the enum is save-view *state*, public API in
/// tango-gamesupport, while the strings live in this crate's bundle.
pub fn library_sort_label(sort: LibrarySort, lang: &LanguageIdentifier) -> String {
    match sort {
        ...
        LibrarySort::Hp => t!(lang, "folder-sort-hp"),
    }
}
...
pub struct GroupedChip {
    ...
    pub has_tag1: bool,
    pub has_tag2: bool,
    /// Raw folder slots represented by this row. Empty for non-folder users
    /// such as Auto Battle Data.
    pub slots: Vec<usize>,
}
...
pub fn chip_row<M: 'static>(
        ...
        r = r.push(
            text(format!("{}×", g.count))
                ...
                .style(move |theme: &iced::Theme| iced::widget::text::Style {
                    color: Some(if illegal {
                        theme.palette().danger
                    } else if count_is_one {
                        muted_color(theme)
                    } else {
                        theme.palette().text
                    }),
    ...
    let card = card_wrap_maybe_danger_text(r.padding([3, 12]).into(), accent, row_idx, illegal);
    ...
    let image_handle = if chip_class == Some(crate::dataview::rom::ChipClass::ProgramAdvance) {
        None
    } else {
...
pub fn class_accent(class: Option<crate::dataview::rom::ChipClass>, dark: bool) -> Option<iced::Color> {
    if dark {
        return Some(iced::Color::from_rgb8(0x4a, 0x55, 0x82));
    }
    match class {
        Some(crate::dataview::rom::ChipClass::Mega) => Some(iced::Color::from_rgb8(0x52, 0x84, 0x9c)),
        Some(crate::dataview::rom::ChipClass::Giga) => Some(iced::Color::from_rgb8(0xc4, 0x52, 0x84)),
        _ => None,
    }
}
...
pub fn chip_popover_with_issue<'a, M: 'a>(
    ...
    if let Some(desc) = description {
        // One text widget per line: a single widget can swallow the
        // newlines depending on shaping, and a description's line
        // breaks are authored (BCC's card text).
...
pub fn with_chip_tooltip_issue<'a>(
    ...
    let is_pa = info
        .as_ref()
        .is_some_and(|i| i.class() == crate::dataview::rom::ChipClass::ProgramAdvance);
...
pub fn as_text(loaded: &OpenSave, opts: RenderOpts) -> Option<String> {
    ...
    let chips: Vec<Option<crate::dataview::save::Chip>> = (0..chips_view.folder_size())
