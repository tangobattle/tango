...
short-save = セーブファイルが短すぎます。
unsupported-save-shift = このセーブデータのシフト値には対応していません。
invalid-checksum = セーブデータのチェックサムが一致しません。
invalid-folder-count = セーブデータには1個から3個のフォルダが必要です。
invalid-slot = フォルダ{ $folder }のスロット{ $slot }：チップIDまたはコードが無効です。

short-rom-header = ROMヘッダーが途中で切れています。
short-chip-table = ROMのチップテーブルが途中で切れています。
