...
unsafe extern "C" fn vfile_seek(
    ...
    f.seek(match whence as u32 {
        mgba_sys::SEEK_SET => std::io::SeekFrom::Start(offset as _),
        mgba_sys::SEEK_CUR => std::io::SeekFrom::Current(offset as _),
        mgba_sys::SEEK_END => std::io::SeekFrom::End(offset as _),
        _ => {
            return -1;
        }
    })
