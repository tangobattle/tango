...
//! Copied from the desktop's `library/rom_overrides.rs` (the crates were
//! deliberately not shared — see the port plan): the wrapper layers on top
//! of any `tango_dataview::rom::Assets` impl (i.e. the per-game assets
//! returned by `tango_gamesupport::Game::load_rom_assets`). Parsed out of
//! each patch version's `info.toml`, the same schema the desktop reads.
