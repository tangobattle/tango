//! Headless round/match lifecycle probe: prime a lockstep pair, force a
//! KO, and drive the games' REAL post-battle flow — result screens back
//! to the comm menu — tracing every telemetry lifecycle event and every
//! primer/diagnostic trap firing along the way.
//!
//! bn1 matches are a single battle (no rematch conversation), so the
...
//! deterministic input pattern — bn1's buster does 1 damage, so the KO
...
/// The primer's state-POKING trap sites (boot fast-path routing). If any
/// of these re-fires after the battle has run, the poke corrupts the
/// games' own post-battle flow — the probe fails on it. The comm-menu
/// init trap is NOT here: the comm menu legitimately re-inits after the
/// battle (the remembered tab holds it for the whole link session) and
/// that trap gates itself inert on the game's own entry flag.
fn poke_anchors(rom: &[u8]) -> &'static [u32] {
    match &rom[0xac..0xb0] {
        b"AREE" => &[0x08018ccc, 0x080105ca, 0x08010498, 0x0800407e],
        b"AREJ" => &[0x08018c40, 0x08010562, 0x08010464, 0x0800406e],
        code => panic!("not a bn1 rom (code {:02x?})", code),
    }
}
            ...
            Phase::PostBattle { since } => {
                ...
                let done = {
                    ...
                    let ended_at = events.iter().find_map(|(et, e)| match e {
                        RoundEvent::Ended { outcome: Some(_) } => Some(*et),
                        _ => None,
                    });
                    let match_ended_at = events
                        .iter()
                        .find_map(|(et, e)| matches!(e, RoundEvent::MatchEnded).then_some(*et));
                ...
                if done {
                    println!("[{t:5}] full lifecycle trace observed; epilogue");
