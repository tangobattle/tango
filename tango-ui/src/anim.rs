//! The motion timelines both sides draw with, plus the activity
//! registry they all report to. Presentation-only: state stays in
//! plain bools / enums, and these wrappers project it through time so
//! overlays and pane swaps ease in instead of popping.
//!
//! Nothing here starts a redraw loop itself — every timeline [`kick`]s
//! the registry when it starts, and the host polls [`any_active`] to
//! decide whether to keep a per-frame subscription alive.
...
/// Process-wide animation clock base for the activity registry.
...
/// toggle, prefer the host's `Overlay`, which bundles the two
/// together.
///
